...
class DRLEnvironment(Node):
    def __init__(self):
        ...
        self.key_pressed_sub = self.create_subscription(Bool, 'key_press', self.key_press_callback, 100)  # Ensure topic matches
    ...
    *******************************************************************************"""
    ...
    def key_press_callback(self, msg):
        self.key_pressed_flag = msg.data  # Update the flag when a key press is detected
    ...
    def step_comm_callback(self, request, response):
        ...
        # Add logging for key press flag
        self.get_logger().info(f'Key pressed flag: {self.key_pressed_flag}')

        # Only publish velocities if key is pressed
        if self.key_pressed_flag:
            twist.linear.x = action_linear
            twist.angular.z = action_angular
            self.get_logger().info(f'Publishing velocities: linear={action_linear}, angular={action_angular}')
        else:
            # If no key press, stop the robot
            twist.linear.x = 0.0
            twist.angular.z = 0.0
            self.get_logger().info('No key press, publishing zero velocities')
...
#         self.running = True
#         self.thread = threading.Thread(target=self.wait_for_key_press)
#         self.thread.start()
...
#         while self.running:
#             key = self.getch()
#             if key == 'q':  # Press 'q' to send the next command
...
#             twist.angular.z = action_angular
#             self.cmd_vel_pub.publish(twist)
#             self.send_command = False  # Reset for next key press
#         else:
...
#     def shutdown(self):
#         self.running = False  # Stop the key press thread
